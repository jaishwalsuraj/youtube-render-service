...
// Railway injects PORT — must bind to it exactly
...
function downloadWithFallback(primary, fallback, dest) {
  return downloadFile(primary, dest)
    .then(() => {
      ...
      if (s.size < 1000) throw new Error('Too small');
      return dest;
    })
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          else reject(new Error('Drive upload failed: ' + data));
        } catch (e) { reject(new Error('Drive parse error: ' + data)); }
      });
    });
    req.on('error', reject);
    req.write(bodyBuf);
...
app.get('/', (req, res) => {
  ...
  res.json({
    status: 'ok',
    service: 'YouTube Render Microservice',
    port: PORT,
...
app.post('/render', async (req, res) => {
  ...
  const {
    run_id, tts_chunks_base64, image_urls, thumbnail_url,
    scene_count, total_seconds, video_title,
    google_access_token, google_drive_folder_id,
  } = req.body;
  ...
  try {
    ...
    if (chunkFiles.length === 1) {
      fs.copyFileSync(chunkFiles[0], voicePath);
    } else {
      execSync(`cat ${chunkFiles.map(f => `"${f}"`).join(' ')} > "${voicePath}"`);
    }
    ...
    try {
      await downloadFile('https://cdn.pixabay.com/download/audio/2022/11/22/audio_febc508520.mp3', musicPath);
    } catch { console.log(`[${run_id}] No music, continuing.`); }
    ...
    try {
      await downloadWithFallback(thumbnail_url, 'https://picsum.photos/1920/1080?random=999', thumbPath);
    } catch {}
    ...
    let concatContent = scenePaths.map((p, i) =>
      `file '${p}'\nduration ${i === count - 1 ? base + extra : base}`
    ).join('\n');
    ...
    try {
      execSync(cmd, { maxBuffer: 50 * 1024 * 1024, timeout: 600000 });
    } catch (e) {
      throw new Error('FFmpeg: ' + String(e.stdout || e.message || '').substring(0, 400));
    }
    ...
    res.json({
      success: true, run_id, elapsed_seconds: parseFloat(elapsed),
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🎬 Render service running on 0.0.0.0:${PORT}`);
  try {
